...
(define (tail-rec-length lst)
  (define (helper h-lst prefix-len)
    (cond
      [(empty? h-lst) prefix-len]
      [else (helper (rest h-lst) (add1 prefix-len))]))
   (helper lst 0))
